...
class UnderDeclaredDutyViewSpec extends SpecBase {

  "UnderDeclaredDutyView" - {
    ...
    "must render the page with the correct content" in new Setup {
      ...
      doc.title must include(messages("underDeclaredDuty.title"))
      doc.select(".govuk-caption-l").text mustBe
        messages("underDeclaredDuty.caption", "1 Jan 2025", "31 Mar 2025")

      doc.select("legend").text must include(messages("underDeclaredDuty.heading"))
    ...
    "must render an error summary when the form has errors" in new Setup {
      ...
      doc.select(".govuk-error-summary__list a").text must include(
        messages("underDeclaredDuty.error.required")
      )
    }
  ...
  trait Setup {
    ...
    val view = app.injector.instanceOf[UnderDeclaredDutyView]
    val form = new UnderDeclaredDutyFormProvider()()
