...
class CalculatedMGDStandardRateViewSpec extends SpecBase {
  ...
  "CalculatedMGDStandardRateView" - {
    ...
    "must render the page with correct heading, caption and input" in new Setup {
      val html = view(form, netTakings, BigDecimal(200), percentage, NormalMode, selectedReturn, None)
    ...
    "must render error summary when form has errors" in new Setup {
      ...
      val html = view(boundForm, netTakings, BigDecimal(200), percentage, NormalMode, selectedReturn, None)
      val doc = Jsoup.parse(html.body)
    ...
    "must display a positive net takings amount correctly in the body" in new Setup {
      val html = view(form, BigDecimal(1000), BigDecimal(200), percentage, NormalMode, selectedReturn, None)
      val doc = Jsoup.parse(html.body)
