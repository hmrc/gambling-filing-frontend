...
class MgdHigherRateViewSpec extends SpecBase {
  ...
  "MgdHigherRateView" - {
    ...
    "must render the page with correct heading, caption and input" in new Setup {
      ...
      doc.title must include(messages("mgdHigherRate.title"))
      doc.select("h1").text mustBe messages("mgdHigherRate.heading")
      doc.select(".govuk-caption-l").text mustBe messages("mgdHigherRate.caption", "1 Jan 2025", "31 Mar 2025")
    ...
    "must render error summary when form has errors" in new Setup {
      ...
      doc.select(".govuk-error-summary__list a").text must include(messages("mgdHigherRate.error.required"))
    }
