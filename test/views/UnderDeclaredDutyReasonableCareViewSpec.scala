...
class UnderDeclaredDutyReasonableCareViewSpec extends SpecBase {

  "UnderDeclaredDutyReasonableCareView" - {
    ...
    "must render the page with the correct content" in new Setup {
      ...
      doc.title must include(messages("underDeclaredDutyReasonableCare.title"))
      doc.select("legend").text mustBe messages("underDeclaredDutyReasonableCare.heading")
      doc.select(".govuk-caption-l").text mustBe
        messages("underDeclaredDutyReasonableCare.caption", "1 Jan 2025", "31 Mar 2025")
    ...
    "must render an error summary when the form has errors" in new Setup {
      ...
      val html = view(boundForm, NormalMode, SelectedReturn(1, LocalDate.of(2025, 1, 1), LocalDate.of(2025, 3, 31)))
      val doc = Jsoup.parse(html.body)

      doc.select(".govuk-error-summary").isEmpty mustBe false
      doc.select(".govuk-error-summary__list a").text must include(
        messages("underDeclaredDutyReasonableCare.error.required")
      )
    }

    "must not render an error summary when the form has no errors" in new Setup {

      val html = view(form, NormalMode, SelectedReturn(1, LocalDate.of(2025, 1, 1), LocalDate.of(2025, 3, 31)))
      val doc = Jsoup.parse(html.body)
  ...
  trait Setup {
    ...
    val view = app.injector.instanceOf[UnderDeclaredDutyReasonableCareView]
    val form = new UnderDeclaredDutyReasonableCareFormProvider()()
