...
class TotalUnderDeclaredDutyViewSpec extends SpecBase {

  "TotalUnderDeclaredDutyView" - {
    ...
    "must render the page with correct heading, caption and input" in new Setup {
      ...
      doc.title must include(messages("totalUnderDeclaredDuty.title"))
      doc.select("h1").text mustBe messages("totalUnderDeclaredDuty.heading")
      doc.select(".govuk-caption-l").text mustBe messages("totalUnderDeclaredDuty.caption", "1 Jan 2025", "31 Mar 2025")
      doc.select(".govuk-input__prefix").text mustBe "£"
      doc.select(".govuk-hint").text mustBe messages("totalUnderDeclaredDuty.hint")
    ...
    "must render error summary when form has errors" in new Setup {
      ...
      doc.select(".govuk-error-summary__list a").text must include(
        messages("totalUnderDeclaredDuty.error.required")
      )
    }

    "must render maximum value error when value exceeds maximum allowed" in new Setup {
      val boundForm = form.bind(Map("value" -> "27001"))
      val html = view(boundForm, NormalMode, selectedReturn)
      val doc = Jsoup.parse(html.body)
  ...
  trait Setup {
    ...
    val view = app.injector.instanceOf[TotalUnderDeclaredDutyView]
