...
class NegativeDutyViewSpec extends SpecBase {
  ...
  "NegativeDutyView" - {
    ...
    "must render the page with the correct content" in new Setup {
      ...
      doc.select("h1").text mustBe messages("negativeDuty.heading")
      doc.select(".govuk-caption-l").text mustBe
        messages("negativeDuty.caption", "1 Jan 2025", "31 Mar 2025")

      doc.select("p.govuk-body").text mustBe
        messages("negativeDuty.p1")

      doc.select("legend").text must include(messages("negativeDuty.question"))
    ...
    "must render an error summary when the form has errors" in new Setup {
      ...
      doc.select(".govuk-error-summary__list a").text must include(
        messages("negativeDuty.error.required")
      )
    }
