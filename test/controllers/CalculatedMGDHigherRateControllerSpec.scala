...
import pages.{CalculatedMGDHigherRatePage, NetTakingsHigherPage, SelectReturnPage}
...
class CalculatedMGDHigherRateControllerSpec extends SpecBase with MockitoSugar {

  private val formProvider = new CalculatedMGDHigherRateFormProvider()
  ...
  private val userAnswersWithNetTakings =
    emptyUserAnswers
      ...
      .success
      .value
      .set(NetTakingsHigherPage, netTakings)
      .success
      .value

  private val userAnswers =
    userAnswersWithNetTakings
      .set(CalculatedMGDHigherRatePage, validAnswer)
      .success
      .value

  def onwardRoute = Call("GET", "/foo")

  lazy val calculatedMGDHigherRateRoute = routes.CalculatedMGDHigherRateController.onPageLoad(NormalMode).url

  "CalculatedMGDHigherRateController" - {

    "must return OK and the correct view for a GET" in {
      val application = applicationBuilder(userAnswers = Some(userAnswersWithNetTakings)).build()

      running(application) {
        val request = FakeRequest(GET, calculatedMGDHigherRateRoute)
        val result = route(application, request).value
        val view = application.injector.instanceOf[CalculatedMGDHigherRateView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, calculatedMGDHigherRateRoute)
