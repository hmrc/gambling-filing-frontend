...
class CalculatedMGDStandardRatePageSpec extends SpecBase {
  ...
  "CalculatedMGDStandardRatePage cleanup" - {

    "must remove the corrected duty amount when set to false" in {
      val result = answersWithAmount.set(CalculatedMGDStandardRatePage, false).success.value

      result.get(MgdStandardRatePage) mustBe None
    }

    "must leave the corrected duty amount untouched when set to true" in {
      val result = answersWithAmount.set(CalculatedMGDStandardRatePage, true).success.value
